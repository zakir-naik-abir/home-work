...
const corsOptions = {
  origin: [
    'http://localhost:5173',
    'http://localhost:5174',
    'https://home-work-af049.web.app',
    'https://https://home-work-af049.firebaseapp.com',
  ],
  credentials: true,
...
const uri = `mongodb+srv://${process.env.DB_USER}:${process.env.DB_PASS}@cluster0.lagpjii.mongodb.net/?appName=Cluster0`;
...
async function run() {
  try {
    const jobsCollection = client.db('homeWork').collection('jobs')
    const bidsCollection = client.db('homeWork').collection('bids')
...
app.get('/', (req, res) => {
  res.send('Hello from HomeWork Server....')
})
